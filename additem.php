...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($nama_barang) && !empty($kategori) && !empty($satuan) && $jumlah_barang >= 0) {
        $stmt = $conn->prepare("INSERT INTO stock (Nama_Barang, Kategori, Jumlah_Barang, Satuan, ID_User, Tanggal_Pembaruan)
                                VALUES (?, ?, ?, ?, ?, NOW())");
        $stmt->bind_param("ssisi", $nama_barang, $kategori, $jumlah_barang, $satuan, $id_user);

        if ($stmt->execute()) {
            $success = "Barang berhasil ditambahkan!";
        } else {
            $error = "Gagal menambah barang. Silakan coba lagi.";
        }
        ...
        <div class="form-container">
            ...
            <form method="POST" action="">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="kategori" name="kategori" placeholder="Example: Food, Beverages, etc"
                        required>
                </div>

                <div class="form-group">
                    <label for="jumlah_barang">Quantity</label>
                    <input type="number" id="jumlah_barang" name="jumlah_barang" placeholder="Enter quantity" min="0"
                        required>
                </div>

                <div class="form-group">
                    <label for="satuan">Unit</label>
                    <input type="text" id="satuan" name="satuan" placeholder="Example: pcs, box, liter..." required>
                </div>
